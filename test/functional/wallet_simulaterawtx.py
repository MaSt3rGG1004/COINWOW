...
class SimulateTxTest(COINWOWTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(w0.getbalance(), 25.0)
        ...
        coinwow_fee = Decimal(funding["fee"])

        # w0 sees fee + 5 btc decrease, w2 sees + 5 btc
        assert_approx(w0.simulaterawtransaction([tx1])["balance_change"], -(Decimal("5") + coinwow_fee))
        ...
        #       wallet                  tx3                             tx4
        #       w0                      -5 - coinwow_fee + 4.9999       -5 - coinwow_fee
        #       w1                      0                               +4.9999
        assert_approx(w0.simulaterawtransaction([tx1, tx3])["balance_change"], -Decimal("5") - coinwow_fee + Decimal("4.9999"))
        assert_approx(w1.simulaterawtransaction([tx1, tx3])["balance_change"], 0)
        assert_approx(w0.simulaterawtransaction([tx1, tx4])["balance_change"], -Decimal("5") - coinwow_fee)
        ...
        tx2 = funding["hex"]
        coinwow_fee2 = Decimal(funding["fee"])
        assert_approx(w0.simulaterawtransaction([tx2])["balance_change"], -(Decimal("10") + coinwow_fee2))
