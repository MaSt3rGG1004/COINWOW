#!/usr/bin/env python3
# Copyright (c) 2014-present The COINWOW Core developers
...
TMPDIR_PREFIX = "coinwow_func_test_"
...
class Binaries:
    """Helper class to provide information about coinwow binaries
    ...
    def node_argv(self):
        "Return argv array that should be used to invoke coinwowd"
        return self._argv("node", self.paths.coinwowd)

    def rpc_argv(self):
        "Return argv array that should be used to invoke coinwow-cli"
        # Add -nonamed because "coinwow rpc" enables -named by default, but coinwow-cli doesn't
        return self._argv("rpc", self.paths.coinwowcli) + ["-nonamed"]

    def tx_argv(self):
        "Return argv array that should be used to invoke coinwow-tx"
        return self._argv("tx", self.paths.coinwowtx)

    def util_argv(self):
        "Return argv array that should be used to invoke coinwow-util"
        return self._argv("util", self.paths.coinwowutil)

    def wallet_argv(self):
        "Return argv array that should be used to invoke coinwow-wallet"
        return self._argv("wallet", self.paths.coinwowwallet)

    def chainstate_argv(self):
        "Return argv array that should be used to invoke coinwow-chainstate"
        return self._argv("chainstate", self.paths.coinwowchainstate)

    def _argv(self, command, bin_path):
        """Return argv array that should be used to invoke the command. It
        either uses the coinwow wrapper executable (if COINWOW_CMD is set), or
        the direct binary path (coinwowd, etc). When bin_dir is set (by tests
        calling binaries from previous releases) it always uses the direct
        path."""
        if self.bin_dir is not None:
            return [os.path.join(self.bin_dir, os.path.basename(bin_path))]
        elif self.paths.coinwow_cmd is not None:
            return self.paths.coinwow_cmd + [command]
        else:
            return [bin_path]


class COINWOWTestMetaClass(type):
    """Metaclass for COINWOWTestFramework.

    Ensures that any attempt to register a subclass of `COINWOWTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'COINWOWTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("COINWOWTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("COINWOWTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class COINWOWTestFramework(metaclass=COINWOWTestMetaClass):
    """Base class for a coinwow test script.

    Individual coinwow test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave coinwowds and test.* datadir on exit or error")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use coinwow-cli instead of RPC for all commands")
    ...
    def get_binary_paths(self):
        ...
        binaries = {
            "coinwowd": "COINWOWD",
            "coinwow-cli": "COINWOWCLI",
            "coinwow-util": "COINWOWUTIL",
            "coinwow-tx": "COINWOWTX",
            "coinwow-chainstate": "COINWOWCHAINSTATE",
            "coinwow-wallet": "COINWOWWALLET",
        }
        ...
        # COINWOW_CMD environment variable can be specified to invoke coinwow
        # wrapper binary instead of other executables.
        paths.coinwow_cmd = shlex.split(os.getenv("COINWOW_CMD", "")) or None
        return paths
    ...
    def shutdown(self):
        ...
        rpc_logger = logging.getLogger("COINWOWRPC")
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a coinwowd test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple coinwowd test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the coinwowd
    ...
    def _start_logging(self):
        ...
        # Format logs the same as coinwowd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in coinwow.conf
    ...
    def skip_if_no_coinwowd_tracepoints(self):
        """Skip the running test if coinwowd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("coinwowd has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_coinwowd_zmq(self):
        """Skip the running test if coinwowd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("coinwowd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if coinwow-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("coinwow-wallet has not been compiled")

    def skip_if_no_coinwow_tx(self):
        """Skip the running test if coinwow-tx has not been compiled."""
        if not self.is_coinwow_tx_compiled():
            raise SkipTest("coinwow-tx has not been compiled")

    def skip_if_no_coinwow_util(self):
        """Skip the running test if coinwow-util has not been compiled."""
        if not self.is_coinwow_util_compiled():
            raise SkipTest("coinwow-util has not been compiled")

    def skip_if_no_coinwow_chainstate(self):
        """Skip the running test if coinwow-chainstate has not been compiled."""
        if not self.is_coinwow_chainstate_compiled():
            raise SkipTest("coinwow-chainstate has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if coinwow-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("coinwow-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether coinwow-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether coinwow-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_coinwow_tx_compiled(self):
        """Checks whether coinwow-tx was compiled."""
        return self.config["components"].getboolean("BUILD_COINWOW_TX")

    def is_coinwow_util_compiled(self):
        """Checks whether coinwow-util was compiled."""
        return self.config["components"].getboolean("ENABLE_COINWOW_UTIL")

    def is_coinwow_chainstate_compiled(self):
        """Checks whether coinwow-chainstate was compiled."""
        return self.config["components"].getboolean("ENABLE_COINWOW_CHAINSTATE")
