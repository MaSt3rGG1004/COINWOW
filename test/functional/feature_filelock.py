...
from test_framework.test_node import (
    COINWOW_PID_FILENAME_DEFAULT,
    ErrorMatch,
)

class FilelockTest(COINWOWTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Check that we can't start a second coinwowd instance using the same datadir")
        ...
        self.log.info("Check that we can't start a second coinwowd instance using the same blocksdir")
        ...
        self.log.info("Check that cookie and PID file are not deleted when attempting to start a second coinwowd using the same datadir/blocksdir")
        cookie_file = datadir / ".cookie"
        assert cookie_file.exists()  # should not be deleted during the second coinwowd instance shutdown
        pid_file = datadir / COINWOW_PID_FILENAME_DEFAULT
        ...
        if self.is_wallet_compiled():
            ...
            self.log.info("Check that we can't start a second coinwowd instance using the same wallet")
