...
class WalletTest(COINWOWTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(self.nodes[0].getbalance("*", 1), 25)
        assert_equal(self.nodes[0].getbalance(minconf=1), 25)
        assert_equal(self.nodes[0].getbalance(minconf=0), 25)
        assert_equal(self.nodes[0].getbalance("*", 1, True), 25)
        assert_equal(self.nodes[1].getbalance(minconf=0), 25)

        # Send 20 CW from 0 to 1 and 30 CW from 1 to 0.
        txs = create_transactions(self.nodes[0], self.nodes[1].getnewaddress(), 20, [Decimal('0.01')])
        self.nodes[0].sendrawtransaction(txs[0]['hex'])
        self.nodes[1].sendrawtransaction(txs[0]['hex'])  # sending on both nodes is faster than waiting for propagation

        self.sync_all()
        txs = create_transactions(self.nodes[1], self.nodes[0].getnewaddress(), 30, [Decimal('0.01'), Decimal('0.02')])
        ...
        # Before `test_balance()`, we have had two nodes with a balance of 25
        # each and then we:
        #
        # 1) Sent 20 from node A to node B with fee 0.01
        # 2) Sent 30 from node B to node A with fee 0.01
        #
        ...
        # a balance of 15 on node B as unconfirmed and trusted.
        #
        ...
        #
        # 1) Sent 20 from node A to node B with fee 0.01
        # 2) Sent 5 from node B to node A with fee 0.01
        #
        # Then our node would report a confirmed balance of 20 + 25 - 5 = 40
        # CW, which is more than would be available if transaction 1 were
        # replaced.
        ...
        def test_balances(*, fee_node_1=0):
            ...
            expected_balances_0 = {'mine':      {'immature':          Decimal('0E-8'),
                                                 'trusted':           Decimal('4.99'),  # change from node 0's send
                                                 'untrusted_pending': Decimal('30.0')}}
            expected_balances_1 = {'mine':      {'immature':          Decimal('0E-8'),
                                                 'trusted':           Decimal('0E-8'),  # node 1's send had an unsafe input
                                                 'untrusted_pending': Decimal('15.0') - fee_node_1}}  # Doesn't include output of node 0's send since it was spent
            ...
            assert_equal(self.nodes[0].getbalance(), Decimal('4.99'))  # change from node 0's send
            assert_equal(self.nodes[1].getbalance(), Decimal('0'))  # node 1's send had an unsafe input
            # Same with minconf=0
            assert_equal(self.nodes[0].getbalance(minconf=0), Decimal('4.99'))
        ...
        balance_node0 = Decimal('34.99')  # node 1's send plus change from node 0's send
        balance_node1 = Decimal('14.98')  # change from node 0's send
        ...
        txs = create_transactions(self.nodes[1], self.nodes[0].getnewaddress(), Decimal('14.97'), [Decimal('0.01')])
        self.nodes[1].sendrawtransaction(txs[0]['hex'])
        ...
        for _ in range(3):
            ...
            txid = self.nodes[0].sendtoaddress(self.nodes[0].getnewaddress(), Decimal('49.5'))
        ...
        tx_replace = tx_orig.replace(
            (4_950_000_000).to_bytes(8, "little", signed=True).hex(),
            (4_850_000_000).to_bytes(8, "little", signed=True).hex(),
        )
