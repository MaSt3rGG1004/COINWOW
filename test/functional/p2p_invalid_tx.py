#!/usr/bin/env python3
# Copyright (c) 2015-2022 The COINWOW Core developers
...
class InvalidTxRequestTest(COINWOWTestFramework):
    ...
    def run_test(self):
        ...
        tx_withhold.vout = [CTxOut(nValue=(25 * COIN) // 2 - 12000, scriptPubKey=SCRIPT_PUB_KEY_OP_TRUE)] * 2

        # Our first orphan tx with some outputs to create further orphan txs
        tx_orphan_1 = CTransaction()
        tx_orphan_1.vin.append(CTxIn(outpoint=COutPoint(tx_withhold.txid_int, 0)))
        tx_orphan_1.vout = [CTxOut(nValue=(8 * COIN) // 2, scriptPubKey=SCRIPT_PUB_KEY_OP_TRUE)] * 3

        # A valid transaction with low fee
        tx_orphan_2_no_fee = CTransaction()
        tx_orphan_2_no_fee.vin.append(CTxIn(outpoint=COutPoint(tx_orphan_1.txid_int, 0)))
        tx_orphan_2_no_fee.vout.append(CTxOut(nValue=(8 * COIN) // 2, scriptPubKey=SCRIPT_PUB_KEY_OP_TRUE))

        # A valid transaction with sufficient fee
        tx_orphan_2_valid = CTransaction()
        tx_orphan_2_valid.vin.append(CTxIn(outpoint=COutPoint(tx_orphan_1.txid_int, 1)))
        tx_orphan_2_valid.vout.append(CTxOut(nValue=(8 * COIN) // 2 - 12000, scriptPubKey=SCRIPT_PUB_KEY_OP_TRUE))

        # An invalid transaction with negative fee
        tx_orphan_2_invalid = CTransaction()
        tx_orphan_2_invalid.vin.append(CTxIn(outpoint=COutPoint(tx_orphan_1.txid_int, 2)))
        tx_orphan_2_invalid.vout.append(CTxOut(nValue=(11 * COIN) // 2, scriptPubKey=SCRIPT_PUB_KEY_OP_TRUE))
        ...
        rejected_parent.vout.append(CTxOut(nValue=(11 * COIN) // 2, scriptPubKey=SCRIPT_PUB_KEY_OP_TRUE))
        ...
        tx_withhold_until_block_A.vout = [CTxOut(nValue=(12 * COIN) // 2, scriptPubKey=SCRIPT_PUB_KEY_OP_TRUE)] * 2
        ...
        tx_orphan_include_by_block_A.vout.append(CTxOut(nValue=(12 * COIN) // 2 - 12000, scriptPubKey=SCRIPT_PUB_KEY_OP_TRUE))
        ...
        tx_withhold_until_block_B.vout.append(CTxOut(nValue=(11 * COIN) // 2, scriptPubKey=SCRIPT_PUB_KEY_OP_TRUE))
        ...
        tx_orphan_include_by_block_B.vout.append(CTxOut(nValue=(10 * COIN) // 2, scriptPubKey=SCRIPT_PUB_KEY_OP_TRUE))
        ...
        tx_orphan_conflict_by_block_B.vout.append(CTxOut(nValue=(9 * COIN) // 2, scriptPubKey=SCRIPT_PUB_KEY_OP_TRUE))
        self.log.info('Send the orphan ... ')
