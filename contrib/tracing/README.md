...
Tracing (USDT) support for COINWOW Core on Linux using. For more information on
USDT support in COINWOW Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `coinwowd` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `coinwowd` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[COINWOW Core USDT documentation] on how to list available tracepoints in your
`coinwowd` binary.

[COINWOW Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py $(pidof coinwowd)
```
...
COINWOW P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py $(pidof coinwowd)
```
...
In a different terminal, starting COINWOW Core in SigNet mode and with
re-indexing enabled.

```
$ ./build/bin/coinwowd -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py $(pidof coinwowd)
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py $(pidof coinwowd)
```
