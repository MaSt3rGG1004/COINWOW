...
Provides utilities for managing copyright headers of `The COINWOW Core
...
Generates a coinwow.conf file in `share/examples/` by parsing the output from `coinwowd --help`. This script is run during the
release process to include a coinwow.conf with the release binaries and can also be run by users to generate a file locally.
...
```bash
BUILDDIR=$PWD/my-build-dir contrib/devtools/gen-coinwow-conf.sh
```
