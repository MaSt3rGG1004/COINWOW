...
be used in COINWOW Core with the `-asmap` option.
...
to binary before being used with COINWOW Core.
...
the `coinwow-cli getnodeaddresses` command.
...
```
coinwow-cli getnodeaddresses 0 > addrs.json
```
