...
signature file SHA256SUMS.asc from coinwowcore.org and coinwow.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/coinwow-core/guix.sigs/tree/main/builder-keys
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the COINWOW community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"coinwow_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by coinwowcore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the coinwow release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes coinwow.org lags behind coinwowcore.org.)')
    )
