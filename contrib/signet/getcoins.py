#!/usr/bin/env python3
# Copyright (c) 2020-2022 The COINWOW Core developers
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing coinwow-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='coinwow-cli', help='coinwow-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='COINWOW address to which the faucet should send')
...
parser.add_argument('coinwow_cli_args', nargs='*', help='Arguments to pass on to coinwow-cli (default: -signet)')

args = parser.parse_args()

if args.coinwow_cli_args == []:
    args.coinwow_cli_args = ['-signet']


def coinwow_cli(rpc_command_and_params):
    argv = [args.cmd] + args.coinwow_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = coinwow_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = coinwow_cli(['getnewaddress', 'faucet', 'bech32'])
