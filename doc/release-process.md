...
* Update coinwow.conf and commit changes if they exist, see [gen-coinwow-conf.sh](/contrib/devtools/README.md#gen-coinwow-confsh).
...
* Update the following variables in [`src/kernel/chainparams.cpp`](/src/kernel/chainparams.cpp) for mainnet, testnet, and signet:
  ...
  - `m_assumeutxo_data` array should be appended to with the values returned by calling `coinwow-cli -rpcclienttimeout=0 -named dumptxoutset utxo.dat rollback=<height or hash>`
...
- Translations on Transifex:
    ...
    - Create [a new resource](https://app.transifex.com/coinwow/coinwow/content/) named after the major version with the slug `qt-translation-<RRR>x`, where `RRR` is the major branch number padded with zeros. Use `src/qt/locale/coinwow_en.xlf` to create it.
    ...
    - Make an announcement that translators can start translating for the new version. You can use one of the [previous announcements](https://app.transifex.com/coinwow/communication/) as a template.
    - Change the auto-update URL for the resource to `master`, e.g. `https://raw.githubusercontent.com/coinwow/coinwow/master/src/qt/locale/coinwow_en.xlf`. (Do this only after the previous steps, to prevent an auto-update from interfering.)
...
Checkout the COINWOW Core version you'd like to build:
...
In the `guix-build-${VERSION}/output/x86_64-apple-darwin` and `guix-build-${VERSION}/output/arm64-apple-darwin` directories:
    ...
    tar xf coinwow-${VERSION}-${ARCH}-apple-darwin-codesigning.tar.gz
...
In the `guix-build-${VERSION}/output/x86_64-w64-mingw32` directory:
    ...
    tar xf coinwow-${VERSION}-win64-codesigning.tar.gz
...
However if this is done, once the release has been tagged in the coinwow-detached-sigs repo, the `guix-codesign` step must be performed again in order for the guix attestation to be valid when compared against the attestations of non-codesigner builds. The directories created by `guix-codesign` will need to be cleared prior to running `guix-codesign` again.

### Windows and macOS codesigners only: Commit the detached codesign payloads

```sh
pushd ./coinwow-detached-sigs
...
- After uploading release candidate binaries, notify the coinwow-core-dev mailing list and
  coinwow-dev group that a release candidate is available for testing. Include a link to the release
  notes draft.
...
- Update repositories
  ...
  - Delete post-EOL [release branches](https://github.com/coinwow/coinwow/branches/all) and create a tag `v${branch_name}-final`.

  - Delete ["Needs backport" labels](https://github.com/coinwow/coinwow/labels?q=backport) for non-existing branches.
