COINWOW Core version 28.0 is now available from:

  <https://coinwowcore.org/bin/coinwow-core-28.0/>
...
Running COINWOW Core binaries on macOS requires self signing.
```
cd /path/to/coinwow-28.0/bin
xattr -d com.apple.quarantine coinwow-cli coinwow-qt coinwow-tx coinwow-util coinwow-wallet coinwowd test_coinwow
codesign -s - coinwow-cli coinwow-qt coinwow-tx coinwow-util coinwow-wallet coinwowd test_coinwow
```
...
Support for Testnet4 as specified in [BIP94](https://github.com/coinwow/bips/blob/master/bip-0094.mediawiki)
...
The default data directory on Windows has been moved from `C:\Users\Username\AppData\Roaming\COINWOW`
to `C:\Users\Username\AppData\Local\COINWOW`. COINWOW Core will check the existence
...
See [JSON-RPC-interface.md](https://github.com/coinwow/coinwow/blob/master/doc/JSON-RPC-interface.md#json-rpc-11-vs-20) for details. (#27101)
...
libcoinwowconsensus Removal
---------------------------

The libcoinwow-consensus library was deprecated in 27.0 and is now completely removed. (#29648)

P2P and Network Changes
-----------------------

- Previously if COINWOW Core was listening for P2P connections, either using
...
- COINWOW Core will now fail to start up if any of its P2P binds fail, rather
...
- Transactions with version number set to 3 are now treated as standard on all networks (#29496),
  ...
  described in [BIP 431](https://github.com/coinwow/bips/blob/master/bip-0431.mediawiki).  The
...
- The `warnings` field in `getblockchaininfo`, `getmininginfo` and
  ...
  can be temporarily restored by running COINWOW Core with the configuration
...
are now required to compile COINWOW Core. (#29091, #30263)

- The minimum required glibc to run COINWOW Core is now
...
- A new `-testdatadir` option has been added to `test_coinwow` to allow specifying the
...
Previous releases of COINWOW Core or previous external software will not be able to read the blocksdir with a non-zero XOR-key.
