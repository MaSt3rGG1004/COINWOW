# CJDNS support in COINWOW Core

It is possible to run COINWOW Core over CJDNS, an encrypted IPv6 network that
...
done outside of COINWOW Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the COINWOW network and individual nodes.
...
before it will work with your COINWOW Core node. This is described in steps
...
## Run COINWOW Core with CJDNS

Once you are connected to the CJDNS network, the following COINWOW Core
...
When enabled, this option tells COINWOW Core that it is running in an
...
IPv6 local network. This helps COINWOW Core perform better address management:
...
CJDNS support was added to COINWOW Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `coinwow-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/coinwow/coinwow/blob/master/doc/tor.md) for
details.

## CJDNS-related information in COINWOW Core

There are several ways to see your CJDNS address in COINWOW Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `coinwow-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `coinwow-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `coinwow-cli help getnodeaddresses` for details.
