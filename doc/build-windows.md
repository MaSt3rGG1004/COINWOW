...
Below are some notes on how to build COINWOW Core for Windows.

The options known to work for building COINWOW Core on Windows are:
...
Note that for WSL the COINWOW Core source path MUST be somewhere in the default mount file system, for
example /usr/src/coinwow, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\coinwow`, for example:
```shell
cmake --install build --prefix /mnt/c/workspace/coinwow
```
...
```shell
cmake --install build --prefix /mnt/c/workspace/coinwow --strip
```
