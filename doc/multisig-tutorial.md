...
Currently, it is possible to create a multisig wallet using COINWOW Core only.

Although there is already a brief explanation about the multisig in the [Descriptors documentation](https://github.com/coinwow/coinwow/blob/master/doc/descriptors.md#multisig), this tutorial proposes to use the signet (instead of regtest), bringing the reader closer to a real environment and explaining some functions in more detail.
...
Before starting this tutorial, start the coinwow node on the signet network.
...
for ((n=1;n<=3;n++))
do
 ./build/bin/coinwow-cli -signet createwallet "participant_${n}"
done
```

`coinwow rpc` can also be substituted for `coinwow-cli`.

Extract the xpub of each wallet. To do this, the `listdescriptors` RPC is used. By default, COINWOW Core single-sig wallets are created using path `m/44'/1'/0'` for PKH, `m/84'/1'/0'` for WPKH, `m/49'/1'/0'` for P2WPKH-nested-in-P2SH and `m/86'/1'/0'` for P2TR based accounts. Each of them uses the chain 0 for external addresses and chain 1 for internal ones, as shown in the example below.
...
All RPCs in COINWOW Core will include the checksum in their output.
...
for ((n=1;n<=3;n++))
do
 xpubs["internal_xpub_${n}"]=$(./build/bin/coinwow-cli -signet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/1/*"))][0] | .desc' | grep -Po '(?<=\().*(?=\))')

 xpubs["external_xpub_${n}"]=$(./build/bin/coinwow-cli -signet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/0/*") )][0] | .desc' | grep -Po '(?<=\().*(?=\))')
done
```
...
external_desc_sum=$(./build/bin/coinwow-cli -signet getdescriptorinfo $external_desc | jq '.descriptor')
internal_desc_sum=$(./build/bin/coinwow-cli -signet getdescriptorinfo $internal_desc | jq '.descriptor')
...
Documentation for these and other parameters can be found by typing `./build/bin/coinwow-cli help importdescriptors`.
...
```bash
./build/bin/coinwow-cli -signet -named createwallet wallet_name="multisig_wallet_01" disable_private_keys=true blank=true

./build/bin/coinwow-cli  -signet -rpcwallet="multisig_wallet_01" importdescriptors "$multisig_desc"

./build/bin/coinwow-cli  -signet -rpcwallet="multisig_wallet_01" getwalletinfo
```
...
```bash
for ((n=1;n<=3;n++)); do ./build/bin/coinwow-cli -signet loadwallet "participant_${n}"; done
```
...
```bash
receiving_address=$(./build/bin/coinwow-cli -signet -rpcwallet="multisig_wallet_01" getnewaddress)

./contrib/signet/getcoins.py -c ./build/bin/coinwow-cli -a $receiving_address
```
...
```bash
./build/bin/coinwow-cli -signet -rpcwallet="multisig_wallet_01" getbalances
```
...
```bash
balance=$(./build/bin/coinwow-cli -signet -rpcwallet="multisig_wallet_01" getbalance)

amount=$(echo "$balance * 0.8" | bc -l | sed -e 's/^\./0./' -e 's/^-\./-0./')

destination_addr=$(./build/bin/coinwow-cli -signet -rpcwallet="participant_1" getnewaddress)

funded_psbt=$(./build/bin/coinwow-cli -signet -named -rpcwallet="multisig_wallet_01" walletcreatefundedpsbt outputs="{\"$destination_addr\": $amount}" | jq -r '.psbt')
```

There is also the `createpsbt` RPC, which serves the same purpose, but it has no access to the wallet or to the UTXO set. It is functionally the same as `createrawtransaction` and just drops the raw transaction into an otherwise blank PSBT. [[source](https://coinwowtalk.org/index.php?topic=5131043.msg50573609#msg50573609)] In most cases, `walletcreatefundedpsbt` solves the problem.
...
```bash
./build/bin/coinwow-cli -signet decodepsbt $funded_psbt

./build/bin/coinwow-cli -signet analyzepsbt $funded_psbt
```
...
```bash
psbt_1=$(./build/bin/coinwow-cli -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq '.psbt')

psbt_2=$(./build/bin/coinwow-cli -signet -rpcwallet="participant_2" walletprocesspsbt $funded_psbt | jq '.psbt')
```
...
```bash
combined_psbt=$(./build/bin/coinwow-cli -signet combinepsbt "[$psbt_1, $psbt_2]")
```
...
```bash
finalized_psbt_hex=$(./build/bin/coinwow-cli -signet finalizepsbt $combined_psbt | jq -r '.hex')

./build/bin/coinwow-cli -signet sendrawtransaction $finalized_psbt_hex
```
...
```bash
psbt_1=$(./build/bin/coinwow-cli -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq -r '.psbt')

psbt_2=$(./build/bin/coinwow-cli -signet -rpcwallet="participant_2" walletprocesspsbt $psbt_1 | jq -r '.psbt')

finalized_psbt_hex=$(./build/bin/coinwow-cli -signet finalizepsbt $psbt_2 | jq -r '.hex')
