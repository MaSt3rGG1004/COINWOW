// Copyright (c) 2019 The COINWOW Core developers
...
Connection::~Connection()
{
    ...
    // object, which can run arbitrary blocking coinwow code so they have to run
    ...
    // functions run, so client objects are fully disconnected before coinwow
    // code in the destructors are run. This way if the coinwow code tries to
...
void Connection::addAsyncCleanup(std::function<void()> fn)
{
    ...
    //
    // In coinwow core, running these callbacks in the right order is
