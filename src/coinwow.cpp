...
Options:
  -m, --multiprocess     Run multiprocess binaries coinwow-node, coinwow-gui.
  -M, --monolithic       Run monolithic binaries coinwowd, coinwow-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show this help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'coinwow-qt [ARGS]' or 'coinwow-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'coinwowd [ARGS]' or 'coinwow-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'coinwow-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'coinwow-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'coinwow-tx [ARGS]'.
...
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_coinwow [ARGS]'.
  chainstate [ARGS] Run coinwow kernel chainstate util, equivalent to running 'coinwow-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_coinwow [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_coinwow-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(cmd.use_multiprocess ? "coinwow-gui" : "coinwow-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(cmd.use_multiprocess ? "coinwow-node" : "coinwowd");
        } else if (cmd.command == "rpc") {
            args.emplace_back("coinwow-cli");
            // Since "coinwow rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("coinwow-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("coinwow-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_coinwow");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("coinwow-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_coinwow");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_coinwow-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("coinwow-util");
        } else {
...
//! Execute the specified coinwowd, coinwow-qt or other command line in `args`
...
//!             be a relative path to a coinwowd, coinwow-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/coinwow ...
...
static void ExecCommand(const std::vector<const char*>& args, std::string_view wrapper_argv0)
{
    ...
    // (https://github.com/coinwow/coinwow/pull/31375#discussion_r1861814807)
