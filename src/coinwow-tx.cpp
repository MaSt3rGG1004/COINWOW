...
static void SetupCOINWOWTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " coinwow-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The coinwow-tx tool is used for creating and modifying coinwow transactions.\n\n"
                "coinwow-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded coinwow transaction, or with \"-create [commands]\" to create a hex-encoded coinwow transaction.\n"
                "\n"
                "Usage: coinwow-tx [options] <hex-tx> [commands]\n"
                "or:    coinwow-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded coinwow transaction
