...
/** Call node.updateRwSetting() with COINWOW 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because COINWOW 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/coinwow/coinwow/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because coinwow 22.x will not try to read these.
...
//! Convert enabled/size values to coinwow -prune setting.
...
//! Get pruning enabled value to show in GUI from coinwow -prune setting.
...
//! Get pruning size value to show in GUI from coinwow -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayCOINWOWUnit")) {
        settings.setValue("DisplayCOINWOWUnit", QVariant::fromValue(COINWOWUnit::CW));
    }
    QVariant unit = settings.value("DisplayCOINWOWUnit");
    if (unit.canConvert<COINWOWUnit>()) {
        m_display_coinwow_unit = unit.value<COINWOWUnit>();
    } else {
        m_display_coinwow_unit = COINWOWUnit::CW;
        settings.setValue("DisplayCOINWOWUnit", QVariant::fromValue(m_display_coinwow_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if coinwow.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_coinwow_unit);
...
{
    if (new_unit.isNull() || new_unit.value<COINWOWUnit>() == m_display_coinwow_unit) return;
    m_display_coinwow_unit = new_unit.value<COINWOWUnit>();
    QSettings settings;
    settings.setValue("DisplayCOINWOWUnit", QVariant::fromValue(m_display_coinwow_unit));
    Q_EMIT displayUnitChanged(m_display_coinwow_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/coinwow/coinwow/pull/8273
    ...
    // (https://github.com/coinwow-core/gui/issues/567).
