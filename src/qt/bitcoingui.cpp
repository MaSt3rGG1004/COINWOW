...
const std::string COINWOWGUI::DEFAULT_UIPLATFORM =
...
COINWOWGUI::COINWOWGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &COINWOWGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &COINWOWGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &COINWOWGUI::showModalOverlay);
...
void COINWOWGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a COINWOW address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and coinwow: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &COINWOWGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &COINWOWGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &COINWOWGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your COINWOW addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified COINWOW addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed COINWOW Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed COINWOW Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a coinwow: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible COINWOW command-line options").arg(CLIENT_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &COINWOWGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &COINWOWGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &COINWOWGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &COINWOWGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &COINWOWGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &COINWOWGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &COINWOWGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &COINWOWGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &COINWOWGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &COINWOWGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &COINWOWGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &COINWOWGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &COINWOWGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &COINWOWGUI::showDebugWindow);
}

void COINWOWGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void COINWOWGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &COINWOWGUI::setCurrentWalletBySelectorIndex);
...
void COINWOWGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &COINWOWGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &COINWOWGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &COINWOWGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &COINWOWGUI::showProgress);
...
#ifdef ENABLE_WALLET
void COINWOWGUI::enableHistoryAction(bool privacy)
{
...
void COINWOWGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &COINWOWGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &COINWOWGUI::removeWallet);
...
WalletController* COINWOWGUI::getWalletController()
{
    return m_wallet_controller;
}

void COINWOWGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &COINWOWGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &COINWOWGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &COINWOWGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &COINWOWGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &COINWOWGUI::incomingTransaction);
    connect(this, &COINWOWGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void COINWOWGUI::removeWallet(WalletModel* walletModel)
{
...
void COINWOWGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void COINWOWGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void COINWOWGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void COINWOWGUI::setWalletActionsEnabled(bool enabled)
{
...
void COINWOWGUI::createTrayIcon()
{
...
void COINWOWGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &COINWOWGUI::toggleHidden);
...
void COINWOWGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void COINWOWGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void COINWOWGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void COINWOWGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void COINWOWGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void COINWOWGUI::openClicked()
{
...
void COINWOWGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void COINWOWGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void COINWOWGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void COINWOWGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void COINWOWGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void COINWOWGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void COINWOWGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void COINWOWGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to COINWOW network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void COINWOWGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void COINWOWGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void COINWOWGUI::updateHeadersSyncProgressLabel()
{
...
void COINWOWGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void COINWOWGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &COINWOWGUI::quitRequested);
...
void COINWOWGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void COINWOWGUI::createWallet()
{
#ifdef ENABLE_WALLET
    auto activity = new CreateWalletActivity(getWalletController(), this);
    connect(activity, &CreateWalletActivity::created, this, &COINWOWGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void COINWOWGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void COINWOWGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &COINWOWGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &COINWOWGUI::show);
                e->ignore();
            }
        }
    }
...
void COINWOWGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void COINWOWGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void COINWOWGUI::incomingTransaction(const QString& date, COINWOWUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(COINWOWUnits::formatWithUnit(unit, amount, true));
...
void COINWOWGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void COINWOWGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool COINWOWGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool COINWOWGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void COINWOWGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void COINWOWGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void COINWOWGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void COINWOWGUI::updateProxyIcon()
{
    std::string ip_port;
...
void COINWOWGUI::updateWindowTitle()
{
...
void COINWOWGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void COINWOWGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void COINWOWGUI::detectShutdown()
{
...
void COINWOWGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void COINWOWGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(COINWOWGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = COINWOWGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void COINWOWGUI::subscribeToCoreSignals()
{
...
void COINWOWGUI::unsubscribeFromCoreSignals()
{
...
bool COINWOWGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<COINWOWUnit> units = COINWOWUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const COINWOWUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, COINWOWUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const COINWOWUnit u : COINWOWUnits::availableUnits()) {
        menu->addAction(COINWOWUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(COINWOWUnit newUnits)
{
    setText(COINWOWUnits::longName(newUnits));
}
