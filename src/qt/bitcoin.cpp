...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaType<COINWOWUnit>("COINWOWUnit");
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. coinwow_de.qm (shortcut "de" needs to be defined in coinwow.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. coinwow_de_DE.qm (shortcut "de_DE" needs to be defined in coinwow.qrc)
...
static const char* qt_argv = "coinwow-qt";

COINWOWApplication::COINWOWApplication()
...
void COINWOWApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the COINWOWApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", COINWOWGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void COINWOWApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool COINWOWApplication::createOptionsModel(bool resetSettings)
{
...
void COINWOWApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new COINWOWGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &COINWOWGUI::quitRequested, this, &COINWOWApplication::requestShutdown);
...
void COINWOWApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void COINWOWApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool COINWOWApplication::baseInitialize()
{
    return node().baseInitialize();
}

void COINWOWApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &COINWOWApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &COINWOWApplication::handleRunawayException);
    connect(this, &COINWOWApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &COINWOWApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void COINWOWApplication::parameterSetup()
{
...
void COINWOWApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void COINWOWApplication::requestInitialize()
{
...
void COINWOWApplication::requestShutdown()
{
...
void COINWOWApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // coinwow: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &COINWOWGUI::handlePaymentRequest);
            connect(window, &COINWOWGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void COINWOWApplication::handleRunawayException(const QString &message)
{
...
void COINWOWApplication::handleNonFatalException(const QString& message)
{
...
WId COINWOWApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool COINWOWApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", COINWOWGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(coinwow);
    Q_INIT_RESOURCE(coinwow_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(COINWOW_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see coinwow-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see coinwow-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse coinwow.conf, determine network, switch to network specific
    ...
    // coinwow: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
