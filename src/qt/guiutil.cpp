// Copyright (c) 2011-present The COINWOW Core developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <qt/guiutil.h>

#include <qt/coinwowaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a COINWOW address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new COINWOWAddressEntryValidator(parent));
    widget->setCheckValidator(new COINWOWAddressCheckValidator(parent));
}
...
bool parseCOINWOWURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no coinwow: URI
    if(!uri.isValid() || uri.scheme() != QString("coinwow"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!COINWOWUnits::parse(COINWOWUnit::CW, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseCOINWOWURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseCOINWOWURI(uriInstance, out);
}

QString formatCOINWOWURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("coinwow:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(COINWOWUnits::format(COINWOWUnit::CW, info.amount, false, COINWOWUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openCOINWOWConf()
{
    ...
    /* Open coinwow.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "COINWOW.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "COINWOW (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("COINWOW (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for COINWOW*.lnk
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "coinwow.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("coinwow-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a coinwow.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=COINWOW\n";
        else
            optionFile << strprintf("Name=COINWOW (%s)\n", ChainTypeToString(chain));
