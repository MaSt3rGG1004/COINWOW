...
const char * const COINWOW_CONF_FILENAME = "coinwow.conf";
const char * const COINWOW_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //coinwow-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", COINWOW_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\COINWOW
    //   new: C:\Users\Username\AppData\Local\COINWOW
    // macOS: ~/Library/Application Support/COINWOW
    // Unix-like: ~/.coinwow
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "COINWOW";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "COINWOW";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/COINWOW";
#else
    // Unix-like
    return pathRet / ".coinwow";
#endif
#endif
}
